...
const StaffServicesTab: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-gray-500 font-medium">Loading your services...</div>;
  }
    ...
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden p-6 md:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage the services you are certified to provide.</p>
        </div>
        <button 
      ...
      {myServices.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
          ...
          <h3 className="text-lg font-bold text-gray-700">No services added yet</h3>
          <p className="text-gray-500 mt-1">Click "+ Add Service" to select the services you offer.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {myServices.map(service => (
            <div key={service.Id} className="border border-emerald-100 bg-gradient-to-b from-emerald-50/40 to-white rounded-2xl p-6 hover:shadow-md transition-all duration-300 group">
              ...
              <div className="space-y-3 mt-4">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-gray-800 font-bold bg-gray-100 px-2.5 py-0.5 rounded-full">{service.DurationMinutes} min</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-gray-500 font-medium flex items-center gap-1.5">
                  ...
                  <span className="text-emerald-600 font-black text-base">${typeof service.Price === 'number' ? service.Price.toFixed(2) : service.Price}</span>
                </div>
                {service.CategoryName && (
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-500 font-medium">Category</span>
                    <span className="text-gray-600 font-semibold">{service.CategoryName}</span>
                  </div>
                )}
              </div>

              <div className="mt-6 pt-4 border-t border-gray-100/80 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button 
                  onClick={() => handleRemove(service.Id)}
                  className="w-full text-sm font-bold text-red-500 bg-red-50 hover:bg-red-500 hover:text-white py-2.5 rounded-xl transition-all duration-200 flex items-center justify-center gap-2"
                >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col animate-fade-in-up">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-2xl font-bold text-gray-800">Available Services</h3>
                <p className="text-gray-500 text-sm mt-1">Select the services you want to add to your profile.</p>
              </div>
              <button 
                  ...
                  <div 
                    ...
                    className={`cursor-pointer rounded-xl p-4 border-2 transition-all flex items-center gap-4 ${
                      selectedInModal.includes(service.Id) 
                        ...
                        : 'border-white bg-white hover:border-emerald-200 hover:bg-emerald-50/30'
                    }`}
                  >
                    <div className={`w-6 h-6 rounded-md border flex items-center justify-center shrink-0 transition-colors ${
                      selectedInModal.includes(service.Id) ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 bg-white'
                    }`}>
                      {selectedInModal.includes(service.Id) && (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500 flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-emerald-600 font-bold">${typeof service.Price === 'number' ? service.Price.toFixed(2) : service.Price}</span>
                      </p>
                    </div>
                  </div>
                ))}
